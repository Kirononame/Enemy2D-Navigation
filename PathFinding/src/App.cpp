		...
		for (int x = 0; x < nMapWidth; x++)
			for (int y = 0; y < nMapHeight; y++)
			{
				...
				FillRect(x*nNodeSize + nNodeBorder, y*nNodeSize + nNodeBorder,
					5, 5, nodes[y * nMapWidth + x].bObstacle ? olc::WHITE : olc::DARK_BLUE);
				...
				if (nodes[y * nMapWidth + x].bVisited)
					FillRect(x*nNodeSize + nNodeBorder, y*nNodeSize + nNodeBorder, 5, 5, olc::BLUE);
				...
				if (&nodes[y * nMapWidth + x] == nodeStart)
					FillRect(x*nNodeSize + nNodeBorder, y*nNodeSize + nNodeBorder, 5, 5, olc::GREEN);
				...
				if (&nodes[y * nMapWidth + x] == nodeEnd)
					FillRect(x*nNodeSize + nNodeBorder, y*nNodeSize + nNodeBorder, 5, 5, olc::RED);
